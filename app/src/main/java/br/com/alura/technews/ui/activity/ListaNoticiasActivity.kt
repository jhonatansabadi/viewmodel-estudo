...
class ListaNoticiasActivity : AppCompatActivity() {
    ...
    private val viewModel by lazy {
        val noticiaDao = AppDatabase.getInstance(this).noticiaDAO
        val repository = NoticiaRepository(noticiaDao)
    ...
    private fun buscaNoticias() {
        val noticiasLiveData = viewModel.buscaTodos()
        noticiasLiveData.observe(this, Observer {
            it.dado?.let {
                adapter.atualiza(it)
            }
            it.erro?.let {
